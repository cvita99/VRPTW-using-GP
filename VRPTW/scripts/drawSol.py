...
def plot_routes(customers, routes, total_distance, instance_name, output_dir):
    ...
    for idx, route in enumerate(routes):
        ...
        plt.plot(x_coords, y_coords, marker='o', label=f'Vozilo {idx+1}', color=colors(idx))

    plt.title(f'{instance_name}\nUkupna udaljenost: {total_distance:.2f}    Broj vozila: {len(routes)}', fontsize=20)
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.grid(True)
    plt.tight_layout()
...
solutions_file = './dynamic/bestDynamicRoutes.txt'
customers_base_dir = './solomonInstances'
output_dir = './dynamic/img'
